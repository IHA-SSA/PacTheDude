...
public class World extends GraphicsProgram {
	...
	public void run() {
		...
		BreadthFirstPaths bfs = new BreadthFirstPaths(labirinth,pac.getX()+pac.getY()*labirinth.X());
		followThePath(bfs.pathTo(aim_x+aim_y*labirinth.Y()));
	...
	private void randomMakesMeCrazy(){
		...
		while (true) {
			...
			if (pac.getY()+y >= 0 && pac.getX()+x>=0 && pac.getY()+y<=labirinth.Y()-1 && pac.getX()+x<=labirinth.X()-1) {
	...
	private void followThePath(Iterable<Integer> iterable){
		...
		for(int k = arr2.size()-1; k >= 0; k--){
			...
			if(prev_step-i==10){
				pac.move(pac.getX(), pac.getY()-1);
				//add(new GLine(last_x*BLOCK_SIZE +BLOCK_SIZE/2, last_y*BLOCK_SIZE +BLOCK_SIZE/2,last_x*BLOCK_SIZE +BLOCK_SIZE/2, pac.getY()*BLOCK_SIZE +BLOCK_SIZE/2));
			}
			//TOP
			if(prev_step-i==-10){
				pac.move(pac.getX(), pac.getY()+1);
			...
			if(prev_step-i==1){
				pac.move(pac.getX()-1, pac.getY());
				//add(new GLine(last_x*BLOCK_SIZE +BLOCK_SIZE/2, last_y*BLOCK_SIZE +BLOCK_SIZE/2,pac.getX()*BLOCK_SIZE +BLOCK_SIZE/2, last_y*BLOCK_SIZE +BLOCK_SIZE/2));
			}
			//RIGHT
			if(prev_step-i==-1){
				pac.move(pac.getX()+1, pac.getY());
	...
	public boolean leftIsClear(int x, int y){
		for(Integer i:labirinth.adj(x)){
	...
	public boolean rightIsClear(int x, int y){
		for(Integer i:labirinth.adj(x)){
			if(i == x+1 + labirinth.X()*y){
				return true;
			}
	...
	public boolean topIsClear(int x, int y){
		for(Integer i:labirinth.adj(x)){
	...
	public boolean bottomIsClear(int x, int y){
		for(Integer i:labirinth.adj(x)){
			if(i == x + labirinth.X()*y + labirinth.X()){
				return true;
			}
